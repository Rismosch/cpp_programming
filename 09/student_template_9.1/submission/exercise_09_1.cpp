 ...
 *
 * @thread_safety This function is fully threadsafe. No modifications were
 * necessary. It only uses variable local to it's thread. It does not share
 * memory, not even globally. Therefore, this method is safe to call in any
 * thread without any synchronisation mechanisms.
 */
 ...
 * @brief Sell a book
 *
 * @thread_safety This function is now full threadsafe. A mutex was added, to
 * protect read-modify-write operations on shared memory. Due to the mutex,
 * read-writes are now done synchronously, avoiding data races.
 */
 ...
 *
 * @thread_safety This function is now full threadsafe. On each loop, it only
 * sells one item. Each operation (sell_toy, or sell_book) is already
 * threadsafe, thus each iteration of the loop is threadsafe as well.
 */
