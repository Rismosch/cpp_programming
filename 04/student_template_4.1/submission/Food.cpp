...
Food::Food(double health_, double power_, double defence_, double health_gain_,
           double dna_gain_)
    : health(health_), power(power_), defence(defence_),
      health_gain(health_gain_), dna_gain(dna_gain_) {}

Food::~Food() {}

void Food::attacked(std::function<void(double &, double)> attack_function) {
  if (attack_function) {
    attack_function(health, defence); // passes the (protected) variables health
                                      // and defence to the function object
  }
}

bool Food::is_alive() { return health > 0; }
