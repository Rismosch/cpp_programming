...
std::vector<size_t> ShortestPaths::compute_shortest_path(size_t from,
  ...
  std::vector<float> dist(adjacency_matrix.size(),
                          std::numeric_limits<float>::infinity());

  // previous node in optimal path from source
  const size_t undefined = ~static_cast<size_t>(0);
  std::vector<size_t> previous(adjacency_matrix.size(), undefined);
  ...
  while (!Q.empty()) {
    ...
    for (size_t v = 0; v < adjacency_matrix.size(); ++v) {
      float dist_between =
          location_u[v].value_or(std::numeric_limits<float>::infinity());
