...
int main() {
  ...
  // load cities
  {
    ...
    for (size_t i = 0; i < graph.size(); ++i) {
      ...
      const float lat =
          std::stof(line.substr(first_comma + 1, second_comma - first_comma));
      const float lon =
          std::stof(line.substr(second_comma + 1, third_comma - second_comma));
      ...
      // produce some approximate values that produce plausible distances in
      // kilometers
  ...
  // add edges between k closest cities
  {
    ...
    auto distance = [](const ShortestPaths::Location &a,
                       const ShortestPaths::Location &b) -> float {
      return std::sqrt((a.pos_x - b.pos_x) * (a.pos_x - b.pos_x) +
    ...
    for (size_t i = 0; i < graph.size(); ++i) {
      ...
      std::nth_element(
          nearest_neighbors.begin(), nearest_neighbors.begin() + nearest_k,
          nearest_neighbors.end(), [](const auto &a, const auto &b) -> bool {
            return std::get<0>(a) < std::get<0>(b);
          });

      // add edges between nearest k elements in both directions
      for (size_t j = 0; j < nearest_k; ++j) {
        graph[i][std::get<1>(nearest_neighbors.at(j))] =
            std::get<0>(nearest_neighbors.at(j));
        graph[std::get<1>(nearest_neighbors.at(j))][i] =
            std::get<0>(nearest_neighbors.at(j));
      }
    }
  }
