...
/// addition onto scalar
inline Point3D operator+(float a, const Point3D &b) { return b + a; }
/// subtraction from scalar
inline Point3D operator-(float a, const Point3D &b) { return Point3D{a} - b; }
/// multiplication onto scalar
inline Point3D operator*(float a, const Point3D &b) { return b * a; }
/// fraction of scalar
inline Point3D operator/(float a, const Point3D &b) { return Point3D{a} / b; }
...
/// check if all components are greater than the other point's components or
/// equal
inline bool operator>=(const Point3D &a, const Point3D &b) {
  return a.x >= b.x && a.y >= b.y && a.z >= b.z;
}
