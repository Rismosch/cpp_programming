...
#include <stdexcept>
  ...
  std::uniform_int_distribution<uint32_t> pick_start{
      0, static_cast<uint32_t>(size() - 1)};
  ...
  for (uint32_t step = 0; step < num_steps; ++step) {
    ...
    const float weight_sum =
        std::accumulate(outgoing.begin(), outgoing.end(), 0.0f,
                        [](float sum, const std::pair<uint32_t, float> &edge) {
...
void RandomWalkGraph::write_histogram_pgm(const std::string &filename,
                                          uint32_t width,
                                          uint32_t height) const {
  const uint64_t num_pixels = static_cast<uint64_t>(width) * height;
  if (num_pixels == 0 || num_pixels != size()) {
    throw std::runtime_error(
        "write_histogram_pgm: resolution does not match the number of nodes");
  }
  ...
  const std::vector<unsigned char> pixels =
      compute_normalized_histogram<unsigned char>(255);
